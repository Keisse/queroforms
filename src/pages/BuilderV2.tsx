...
function saveLocalDraft(steps:Step[]){
  window.localStorage.setItem(LOCAL_DRAFT_KEY, JSON.stringify(steps));
}
...
function sameSteps(a:Step[], b:Step[]){
...
export default function BuilderV2(){
  ...
  const dropStep = (s:Step,e:ReactDragEvent<HTMLDivElement>)=>{
    ...
    setDropTarget(null);
    setSavedMsg('Ordem alterada. Salve a edição da tela selecionada para guardar o rascunho.');
  };
  ...
  const confirmDelete = ()=>{
    ...
    setDeleteIdx(null);
    setConfirmText('');
    setSavedMsg('Tela removida. Salve a edição da tela selecionada para guardar essa alteração.');
  };
  ...
  return <>
    ...
    <p className="muted" style={{margin:'-10px 0 18px'}}>Salve cada tela editada no navegador. A página pública só muda quando você clicar em <b>Publicar</b>, que grava a versão completa no Supabase.</p>
    ...
    <div className="builder-grid" style={{gridTemplateColumns:`${flowWidth}px 12px minmax(300px,1fr) 12px ${propsWidth}px`}}>
      <section className="steps-panel">
        ...
        <div className="steps-list">
          {steps.map((s,i)=>{
            ...
            return <div className={`step-item ${i===sel?'active':''} ${draggedStepId===s.id?'builder-step-dragging':''} ${dropClass}`} key={s.id} onDragOver={e=>dragOverStep(s,e)} onDrop={e=>dropStep(s,e)} style={{cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
        ...
        <div className="builder-add-screen-wrap">
          ...
          <small>Arraste pelo ícone ⋮⋮ para reordenar. Depois salve a edição da tela antes de publicar.</small>
        </div>
      </section>
      ...
      <section className="builder-canvas">
        <div className="canvas-phone">
          ...
          <div className="canvas-inner">
            ...
            {step.kind==='insight' && <><h2>{step.title}</h2><p style={{textAlign:'center',color:'#7a8b9c'}}>{step.body}</p>{step.stat&&<div className="option-card"><b>{step.stat}</b></div>}</>}
            {step.kind==='intro' && <div className="builder-intro-preview"><div className="builder-certificate-image-wrap"><img src="/certificado-final.svg" alt="Certificado Gestão de Projetos com IA - Formação Mestre GP" /></div><h2>{step.title}</h2><p className="builder-intro-question">{step.body}</p><div className="builder-intro-choices"><div>Sim <span>→</span></div><div>Não <span>→</span></div></div></div>}
      ...
      <aside className="props-panel">
        ...
        {step.kind==='question' && <>
          ...
          <label>Opções</label>
          {step.options.map((o,i)=><div key={i} style={{display:'flex',gap:6,marginBottom:6}}><input style={{width:36}} value={o.emoji||''} placeholder="🙂" onChange={e=>updateOption(i,{emoji:e.target.value})}/><input style={{flex:1}} value={o.label} onChange={e=>updateOption(i,{label:e.target.value})}/><button className="btn" onClick={()=>removeOption(i)} title="Remover opção">✕</button></div>)}
        ...
        {step.kind==='insight' && <>
          ...
          <label>Fonte</label><textarea value={step.source||''} onChange={e=>update({source:e.target.value} as Partial<Step>)}/>
        ...
        {step.kind==='intro' && <>
          <label>Título</label><textarea value={step.title} onChange={e=>update({title:e.target.value} as Partial<Step>)}/>
          <label>Pergunta</label><textarea value={step.body} onChange={e=>update({body:e.target.value} as Partial<Step>)}/>
        ...
        {step.kind!=='result' && <div style={{marginTop:20,paddingTop:16,borderTop:'1px solid #e3eaf0'}}>
          <button className="btn dark" data-qf-save-screen="true" onClick={saveCurrentScreen} style={{width:'100%',justifyContent:'center'}}>{currentScreenSaved?'Salvar edição desta tela':'Salvar edição desta tela *'}</button>
    ...
    <div className="builder-resize-hint">↔ Arraste as divisórias entre os painéis para ajustar os tamanhos. A preferência de largura fica salva neste navegador.</div>

    {addOpen && <div className="builder-add-overlay" onClick={()=>setAddOpen(false)}><div className="builder-add-modal" onClick={e=>e.stopPropagation()}><div className="builder-add-modal-head"><div><small>Nova tela</small><h3>O que você quer adicionar?</h3></div><button className="btn" onClick={()=>setAddOpen(false)}>✕</button></div><p className="muted">A nova tela será inserida depois de <b>{sel+1}. {labelFor(step)}</b>. Se o resultado estiver selecionado, ela entra imediatamente antes dele.</p><div className="builder-screen-types">{NEW_SCREEN_OPTIONS.map(item=><button key={item.type} className="builder-screen-type" onClick={()=>addScreen(item.type)}><span>{item.icon}</span><div><b>{item.title}</b><small>{item.description}</small></div></button>)}</div></div></div>}

    {deleteIdx!==null && <div style={{position:'fixed',inset:0,background:'rgba(15,30,50,.45)',display:'grid',placeItems:'center',zIndex:50}} onClick={cancelDelete}><div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:16,padding:28,width:380,display:'flex',flexDirection:'column',gap:12}}><h3 style={{margin:0}}>Excluir esta tela?</h3><p className="muted" style={{margin:0}}>Você está prestes a excluir "<b>{labelFor(steps[deleteIdx])}</b>". Digite <b>EXCLUIR</b> para confirmar.</p><input autoFocus value={confirmText} onChange={e=>setConfirmText(e.target.value)} placeholder="EXCLUIR" style={{padding:12,border:'1px solid #d0dbe3',borderRadius:10}}/><div style={{display:'flex',gap:8,justifyContent:'flex-end'}}><button className="btn" onClick={cancelDelete}>Cancelar</button><button className="btn dark" disabled={confirmText!=='EXCLUIR'} onClick={confirmDelete} style={confirmText!=='EXCLUIR'?{opacity:.5,cursor:'not-allowed'}:{background:'#c0392b',borderColor:'#c0392b'}}>Excluir tela</button></div></div></div>}
  </>;
}
