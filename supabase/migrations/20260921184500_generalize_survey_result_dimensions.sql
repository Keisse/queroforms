...
begin
  ...
  if not exists (
    ...
    raise exception 'survey requires at least one fully scoreable question' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where step->>'kind'='question'
      ...
      and coalesce(btrim(step->>'dimension'),'')=''
  ) then
    raise exception 'every fully scoreable question requires a dimension' using errcode = '22023';
  end if;
